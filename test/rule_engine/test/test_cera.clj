...
(deftest within-test-1
  (let [r1 (within (a-sig 1) 5000)
    ...
    (is (= (date-sec 1) (:start (:status r2a))))
    (is (= (date-sec 5) (:finish (:status r2a))))
    (is (ignore? (:status r2b)))
    (is (nil? (:start (:status r2b))))
    (is (= (date-sec 6) (:finish (:status r2b))))
    (is (futile? (:status r3b)))
    (is (= (date-sec 1) (:start (:status r3b))))
    (is (= (date-sec 10) (:finish (:status r3b))))))
...
(deftest within-test-2
  (let [r1 (within (in-order (a-sig "foo") (a-sig "bar")) 10000)
    ...
    (is (ignore? (:status r2)))
    (is (nil? (:start (:status r2))))
    (is (= (date-sec 10) (:finish (:status r2))))
    (is (active? (:status r3)))
    (is (= (date-sec 1) (:start (:status r3))))
    (is (= (date-sec 10) (:finish (:status r3))))
    (is (ignore? (:status r4)))
    (is (= (date-sec 1) (:start (:status r4))))
    (is (= (date-sec 11) (:finish (:status r4))))
    (is (futile? (:status r5)))
    (is (= (date-sec 1) (:start (:status r5))))
    (is (= (date-sec 20) (:finish (:status r5))))))
...
(deftest within-test-3
  (let [r1 (within (all (a-sig 1)
    ...
    (is (active? (:status r2)))
    (is (= (date-sec 5) (:start (:status r2))))
    (is (= (date-sec 10) (:finish (:status r2))))
    (is (ignore? (:status r3)))
    (is (active? (:status r4)))
    (is (complete? (:status r5)))
    (is (= (date-sec 5) (:start (:status r5))))
    (is (= (date-sec 13) (:finish (:status r5))))))
...
(deftest without-test-1
  (let [r1 (without (a-sig 1) (date-sec 5) (date-sec 10))
    ...
    (is (complete? (:status r2a)))
    (is (= (date-sec 1) (:start (:status r2a))))
    (is (= (date-sec 4) (:finish (:status r2a))))
    (is (ignore? (:status r2b)))
    (is (nil? (:start (:status r2b))))
    (is (= (date-sec 6) (:finish (:status r2b))))
    (is (futile? (:status r3b)))
    (is (= (date-sec 1) (:start (:status r3b))))
    (is (= (date-sec 5) (:finish (:status r3b))))))
...
(deftest without-test-2
  (let [r1 (without (within (in-order (a-sig "foo")
    ...
    (is (futile? (:status r5)))
    (is (= (date-sec 1) (:start (:status r5))))
    (is (= (date-sec 9) (:finish (:status r5))))))
...
(deftest without-test-3
  (let [r1 (without (all (a-sig 1)
    ...
    (is (futile? (:status r5)))
    (is (= (date-sec 5) (:start (:status r5))))
    (is (= (date-sec 16) (:finish (:status r5))))))
